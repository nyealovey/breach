...
| AC-03   | FR-02 Run：支持每日一次定时采集与手动触发；同 Source 单飞 + 触发抑制可审计                       | FR-01.A、FR-02、NFR-03（可靠性） | `POST /sources/:id/runs`、`GET /runs`、`GET /runs/:id`、`POST /runs/:id/cancel` | `schedule_group`、`run`、`audit_event`                              | `schedule_group.triggered`、`run.finished`、`http.request` | 调度组管理、Run 列表/详情、取消      |
...
| AC-08   | OpenAPI/Swagger：提供 OpenAPI JSON 与 Swagger UI，并覆盖 UI 所需 API                             | PRD-only（交付物）               | `GET /openapi.json`、`GET /swagger`                                             | -                                                                   | `http.request`                                             | Swagger UI                           |
