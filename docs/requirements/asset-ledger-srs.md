...
现有虚拟化/云平台资产分散在多个来源（阿里云、Hyper-V、vCenter、PVE 等），并可能因迁移/复制导致“同一资产在不同来源、同一来源不同时间窗口出现多份记录”。需要一个统一台账系统，按固定频率持续采集并留痕，支持人工治理（疑似重复提示与合并），并提供台账侧业务补录字段用于盘点与治理。
...
- 可补录：提供**预设台账字段（固定字段集）**与字段值管理（后续不允许修改字段集合与类型，仅允许维护值）。
...
- 台账字段：提供**预设字段集**（仅 `vm + host`），管理员可在资产上维护字段值（支持批量维护），字段集合与类型后续不变。
- 权限：管理员/普通用户两类角色。
...
- 管理员可访问“来源管理/重复中心/台账字段维护/导出”等管理功能；普通用户不可见且无权限操作。
...
- Source 删除为“软删除”：不物理删除历史数据；删除后默认不可见且不再参与调度/触发；系统不要求提供恢复（restore）能力。
...
- **冲突字段默认选用策略**：当单值字段存在多来源不一致时，默认选用“最新一次成功 `mode=collect` Run”所观察到的值（以 `run.finished_at`/`run.started_at` 作为时间序，取最新）；若最新来源该字段缺失，则回退到下一条更早的成功快照。
...
系统提供一组**预设台账字段（固定字段集）**用于业务补录与盘点治理：

- 字段集合与类型固定：后续版本**不允许新增/删除/停用/改类型**（仅允许维护字段值）。
- 字段按资产类型生效：仅覆盖 `vm + host`；`host` 专用字段不得挂载到 `vm`。
- 字段值变更必须可审计、可追溯（审计永久保留，参见 FR-10）。

**验收标准**

- Given 管理员查看任一资产详情
  When 展示
  Then 资产详情必须展示“台账字段（预设）”区块，并仅展示该资产类型适用的字段。
- Given 管理员编辑台账字段值并保存
  When 保存成功
  Then 字段值被持久化，且写入一条审计事件（含操作者、时间、变更摘要、requestId）。
- Given 管理员对一批资产执行批量维护台账字段
  When 批量更新成功
  Then 所选资产的字段值被一致更新，且写入一条审计事件（含字段、值摘要、影响资产清单、requestId）。
- Given 普通用户访问资产详情
  When 展示
  Then 可只读查看台账字段值；尝试写入相关 API 必须返回 403。
...
- Given 任一用户访问资产列表
  When 展示
  Then 必须支持分页；支持按 `asset_type`、`status`、`source` 等过滤；资产列表默认展示列包含：主机名、虚拟机名、IP、CPU、内存、总分配磁盘、状态、操作；**不展示** last_seen 与 来源；支持关键字搜索（覆盖：主机名/虚拟机名、externalId、uuid 等文本字段，且必须覆盖台账字段（预设）中的可搜索字段，例如：地区/公司/部门/系统分类/系统分级/业务对接人员，以及 host 专用的固定资产编号/管理IP 等；支持空格分词并采用 AND 语义；每个词使用不区分大小写的“模糊包含”匹配；不做中文分词/同义词/拼写容错）。
- Given 用户在资产列表输入关键字 `q="foo bar"`
  When 执行搜索
  Then 系统仅返回同时命中 `foo` 与 `bar` 的资产（AND）。
- Given 用户在资产列表的搜索框输入关键字（打字实时搜索）
  When 用户停止输入
  Then UI 应在 ≤ 300ms 内自动触发一次查询并更新列表结果（无需点击“搜索”按钮）。
- Given 用户在资产列表选择排序
  When 生效
...
| 操作类型                      | 目标响应时间 | 说明                                 |
| ----------------------------- | ------------ | ------------------------------------ |
| 列表查询（分页 + 关键字搜索） | ≤ 500ms      | Source/Run/Asset 列表，单页 ≤ 100 条 |
...
| v2.1 | 台账字段（预设）+ 导出（FR-06/FR-11） | 规划中   | 固定字段集的补录与数据导出 |
