...
- VM 配置规格（配置值，不取实时 usage）：
  ...
  - `normalized.hardware.disks[]`（每块盘明细：name/size_bytes/type；type=provisioning：thin/thick/eagerZeroedThick）
...
#### 2.3 detect 与 collect 的协作

- detect 返回 `compatible: false` 时，UI 应**默认阻止** collect 运行（例如禁用运行按钮），并提示管理员调整 `preferred_vcenter_version` 或升级 vCenter。
- collect 仍需自校验并失败，确保即使绕过 UI/跳过 detect 也能正确拒绝不兼容的版本范围。
...
- VM：
  - 必须：`external_id`、`normalized.identity.hostname`（可用 name/guest hostname 等来源组合）、`normalized.identity.machine_uuid`（若可得）、`normalized.network.mac_addresses[]`（若可得）
  - 必须新增：`normalized.hardware.cpu_count`、`normalized.hardware.memory_bytes`、`normalized.hardware.disks[]`、`normalized.runtime.power_state`
- Host：
  - 必须：`external_id`、`normalized.identity.hostname`（可得则填）、`normalized.identity.serial_number`（可得则填）
- Cluster：
  - 必须：`external_id`、`normalized.identity.caption`（name）
...
- 在 `normalized-v1` 增加结构化字段：
  ...
  - `hardware.disks[].type`（VM 专用；表示磁盘 provisioning 类型）

`runtime.power_state` 枚举值（硬性）：

- `poweredOn`
- `poweredOff`
- `suspended`

`hardware.disks[].type` 枚举值（硬性）：
...
| 指标             | 目标值 | 说明                                                                                                                 |
| ---------------- | ------ | -------------------------------------------------------------------------------------------------------------------- |
| 采集成功率       | ≥ 99%  | 在版本范围匹配的前提下，collect 应稳定成功                                                                           |
| 关系边存在率     | 100%   | collect 成功时 `relations.length > 0`（不要求每个 VM 必须有 runs_on）                                                |
| 字段完整率       | 100%   | 每个 VM 必须有 `normalized.hardware.cpu_count`、`normalized.hardware.memory_bytes`、`normalized.runtime.power_state` |
| 版本不匹配检出率 | 100%   | 版本范围与 vCenter 不兼容时必须失败，不得静默成功                                                                    |
...
- [ ] VM 输出包含配置规格：`normalized.hardware.cpu_count`、`normalized.hardware.memory_bytes`、`normalized.hardware.disks[]`（盘明细：name/size_bytes/type；type=provisioning）。
- [ ] VM 输出包含电源状态：`normalized.runtime.power_state`（枚举：poweredOn / poweredOff / suspended）。

### Quality Standards

- [ ] 文档同步：更新 `README.md` 与 `docs/design/asset-ledger-collector-reference.md`，移除"允许降级"的口径，改为"版本范围 + 不匹配直接失败"。
- [ ] Schema 同步：更新 `src/lib/schema/normalized-v1.schema.json` 与 `docs/design/asset-ledger-json-schema.md`，加入 `runtime.power_state`，并明确 `hardware.disks[].type` 的 provisioning 枚举。

### Test Strategy

- [ ] 单元测试：Driver 选择逻辑、关系边构建逻辑覆盖率 ≥ 80%
- [ ] 集成测试：至少覆盖 vCenter 6.7 和 7.0 两个版本的 mock 场景
- [ ] E2E 测试：验证 UI 必填校验、采集失败提示展示
- [ ] 回归测试：确保历史 Source 在未补齐 `preferred_vcenter_version` 时无法运行 collect
...
- [ ] 验证：资产详情页 Outgoing Relations 区域显示 VM→Host（runs_on）关系
- [ ] 验证：资产详情页 Outgoing Relations 区域显示 Host→Cluster（member_of）关系
- [ ] 验证：资产详情页 Fields 区域显示 `fields.hardware.cpu_count`、`fields.hardware.memory_bytes`、`fields.hardware.disks[]`
- [ ] 验证：资产详情页 Fields 区域显示 `fields.runtime.power_state` 字段
- [ ] 回归：历史 Source 编辑页提示补齐 `preferred_vcenter_version`，未补齐时禁止运行 collect
- [ ] 回归：版本范围不匹配时，Run 状态为 Failed，错误信息包含 `VCENTER_API_VERSION_UNSUPPORTED`
