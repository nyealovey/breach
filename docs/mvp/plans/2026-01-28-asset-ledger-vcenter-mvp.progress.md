...
- 调度组/来源绑定口径：创建/编辑 Source 不要求选择调度组；调度组创建/编辑通过 `sourceIds` 多选来管理组内来源（最终落库仍为 `source.scheduleGroupId`）。
...
7. Task 7：调度组 API + UI（含 scheduler 事件）
   - 状态：DONE
   - 补充修正：调度组创建/编辑在调度组侧选择来源（多选）；创建时要求至少选择 1 个来源，并将所选来源绑定到该组

8. Task 8：Source API + UI（软删除、最近 Run 摘要、凭据更新）
   - 状态：DONE
   - 补充修正：来源创建/编辑不再要求选择调度组（默认不绑定）；调度组成员关系在调度组页面维护
