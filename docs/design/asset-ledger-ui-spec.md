...
资产详情页顶部“盘点摘要”三列表格中（字段 / 采集值 / 覆盖值）：

- 采集值：必须展示**采集值（collected）**，不得回填覆盖值。
- 覆盖值：仅展示用户维护的 override（无则 `-`）。
- 机器名 / 操作系统 / IP 的左侧边框状态（灰/红/蓝/绿）继续基于“覆盖值 vs 采集值”关系判定，不改变语义。

### 信号状态（监控 / 备份）

监控与备份属于“信号来源（signal）”的聚合状态，不属于 canonical 字段，也不遵循“采集值/覆盖值”的双层语义：

- 资产列表页（`/assets`）：
  - “监控”列同时展示两枚小图标：
    - `SW`：SolarWinds 监控状态（正常/告警/宕机/未覆盖/未知）
    - `V`：Veeam 备份状态（成功/告警/失败/未覆盖/未知）
  - 图标按状态染色，tooltip 展示更新时间与简要详情。
- 资产详情页（`/assets/[uuid]`）：
  - 盘点摘要表新增“监控”“备份”行（第三列固定 `-`）。
  - 新增「最近 7 次备份」表格（仅展示最近 7 条，不提供完整备份列表）。
