...
import type { AssetDetailTab } from '@/lib/assets/page-data';
...
export default function AssetDetailPage({ initialData }: { initialData: AssetDetailPageInitialData }) {
  ...
  const debugSourceLoadingRef = useRef(false);

  const [activeTab, setActiveTab] = useState<AssetDetailTab>(() => parseAssetDetailTab(initialData.tab));
  const [asset, setAsset] = useState<AssetDetail | null>(initialData.asset);
  const [debugSourceRecords, setDebugSourceRecords] = useState<SourceRecordItem[]>([]);
  const [debugSourceLoaded, setDebugSourceLoaded] = useState(false);
  const [debugSourceLoading, setDebugSourceLoading] = useState(false);
  const [debugSourceError, setDebugSourceError] = useState<string | null>(null);
  ...
  useEffect(() => {
    setActiveTab(parseAssetDetailTab(initialData.tab));
  }, [initialData.tab, uuid]);

  useEffect(() => {
    setDebugSourceRecords([]);
    setDebugSourceLoaded(false);
    setDebugSourceLoading(false);
    setDebugSourceError(null);
    setShowAllSources(false);
    debugSourceLoadingRef.current = false;
  }, [uuid]);

  const setActiveTabWithUrl = (nextTab: AssetDetailTab) => {
    ...
    const nextUrl = search.length > 0 ? `${url.pathname}?${search}${url.hash}` : `${url.pathname}${url.hash}`;
    window.history.replaceState(window.history.state, '', nextUrl);
  };

  const loadDebugSourceRecords = useCallback(
    async (signal?: AbortSignal) => {
      if (debugSourceLoadingRef.current) return;
      debugSourceLoadingRef.current = true;
      setDebugSourceLoading(true);
      setDebugSourceError(null);

      try {
        const res = await fetch(`/api/v1/assets/${encodeURIComponent(uuid)}/source-records`, { signal });
        if (signal?.aborted) return;

        const body = (await res.json().catch(() => null)) as ApiBody<SourceRecordItem[]> | null;
        if (!res.ok) {
          setDebugSourceError(body?.error?.message ?? `加载失败（${res.status}）`);
          return;
        }

        const records = Array.isArray(body?.data) ? body.data : [];
        setDebugSourceRecords(records);
        setDebugSourceLoaded(true);
      } catch (err) {
        if (signal?.aborted || (err instanceof DOMException && err.name === 'AbortError')) return;
        setDebugSourceError(err instanceof Error ? err.message : String(err));
      } finally {
        debugSourceLoadingRef.current = false;
        setDebugSourceLoading(false);
      }
  ...
  useEffect(() => {
    if (activeTab !== 'debug' || debugSourceLoaded) return;
    const controller = new AbortController();
    void loadDebugSourceRecords(controller.signal);
    return () => controller.abort();
  }, [activeTab, debugSourceLoaded, loadDebugSourceRecords]);
  ...
  const sourceSummaries = useMemo<SourceSummary[]>(() => {
    ...
    for (const r of debugSourceRecords) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid gap-6 ${activeTab === 'overview' ? 'lg:grid-cols-2' : ''}`}>
        {activeTab === 'overview' ? (
          <div className="space-y-6">
            <Card>
              <CardHeader>
                  ...
                  <TableBody>
                    ...
                    {asset.assetType === 'vm' ? (
                      <TableRow>
                        ...
                        <TableCell>
                          {summary.toolsRunning === null ? '-' : summary.toolsRunning ? '是' : '否'}
                        </TableCell>
                        <TableCell>-</TableCell>
                      </TableRow>
                    ) : null}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {allowedLedgerFieldMetas.map((meta) => {
                        const value = asset.ledgerFields?.[meta.key] ?? {
                          source: null,
                          override: null,
                          effective: null,
                        };
                              ...
                              {ledgerEditing && isAdmin ? (
                                meta.kind === 'date' ? (
                                  <Input
                                    ...
                                    value={draft}
                                    onChange={(e) =>
                                      setLedgerDraft((prev) => ({ ...prev, [meta.key]: e.target.value }))
                                    }
                                  />
                                ) : (
                                  <Input
                    ...
                    <div className="space-y-3">
                      {historyItems.map((e) => {
                        ...
                        if (e.eventType === 'collect.changed') {
                          ...
                          const actor =
                            summaryObj?.actor && typeof summaryObj.actor === 'object'
                              ? (summaryObj.actor as any)
                              : null;
                          ...
                          const valueSummary =
                            typeof summaryObj?.valueSummary === 'string' ? summaryObj.valueSummary : '';
                  ...
                  {!debugSourceLoaded && debugSourceLoading ? (
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-muted/20 p-3">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        disabled={debugSourceLoading}
                        onClick={() => void loadDebugSourceRecords()}
                      >
                        {debugSourceLoading ? '重试中…' : '重试'}
                      </Button>
                    </div>
                  ) : !debugSourceLoaded ? (
