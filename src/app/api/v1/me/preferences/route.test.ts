...
describe('GET /api/v1/me/preferences', () => {
  ...
  it('supports assets.table.columns.v2', async () => {
    (requireUser as any).mockResolvedValue({ ok: true, requestId: 'req_test', session: { user: { id: 'u1' } } });
    (prisma.userPreference.findUnique as any).mockResolvedValue({
      id: 'pref_2',
      userId: 'u1',
      key: 'assets.table.columns.v2',
      value: { visibleColumns: ['machineName', 'status', 'ip'] },
    });

    const req = new Request('http://localhost/api/v1/me/preferences?key=assets.table.columns.v2');
    const res = await GET(req);
...
describe('PUT /api/v1/me/preferences', () => {
  ...
  it('upserts v2 preference and returns 200', async () => {
    (requireUser as any).mockResolvedValue({ ok: true, requestId: 'req_test', session: { user: { id: 'u1' } } });
    ...
    expect(prisma.userPreference.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { userId_key: { userId: 'u1', key: 'assets.table.columns.v2' } },
      }),
    );
